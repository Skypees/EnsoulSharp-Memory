...
namespace SupportAIO.Champions
{
    class Lulu : Champion
    {
        ...
        protected override void SemiR()
        {
            ...
            {
               foreach (var ally in GameObjects.AllyHeroes.Where(
                                x => x.Distance(Player) <= W.Range && x.IsAlly && !x.IsMe && !x.IsRecalling() && x.HealthPercent <=
                                     RootMenu["we"][x.CharacterName.ToLower() + "priority"].GetValue<MenuSlider>().Value)
                            .OrderByDescending(x => x.TotalAttackDamage))
                        {
                            if (ally != null && !ally.IsDead && W.IsReady() && ally.CountEnemyHeroesInRange(1090) >= 1)
                            {
                                W.CastOnUnit(ally);
                                }
                if (ally != null && !ally.IsDead && E.IsReady() && ally.CountEnemyHeroesInRange(1090) >= 1)
                            {
                                E.CastOnUnit(ally);
                                }
        ...
        protected override void SetMenu()
        {
            ...
            if (language != 1)
            {
                ...
                foreach (var target in GameObjects.AllyHeroes)
                {
                    ...
                    WE.Add(new MenuSlider(target.CharacterName.ToLower() + "priority", target.CharacterName + " 优先级: ", 20, 0, 100));
            ...
            else
            {
                ...
                WE.Add(new MenuBool("key", "W > E Key"));

                WE.Add(new MenuSeparator("meow", "0 - Disabled"));
                WE.Add(new MenuSeparator("meowmeow", "1 - Lowest, 100 - Biggest Priority"));
                foreach (var target in GameObjects.AllyHeroes)
                {

                    WE.Add(new MenuSlider(target.CharacterName.ToLower() + "priority", target.CharacterName + " Priority: ", 20, 0, 100));
